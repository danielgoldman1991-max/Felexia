        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4 text-sm lg:grid-cols-4">
            ...
            <div><span className="text-[var(--muted)]">Compte parent:</span> {
              detailResult?.parent
                ? <Link href={`/comptabilite/comptes/${detailResult.parent.id}`} className="text-indigo-700 hover:underline">{detailResult.parent.code} - {detailResult.parent.name}</Link>
                : "-"
            }</div>
