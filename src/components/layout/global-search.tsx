  ...
  return (
    <div ref={containerRef} className="relative min-w-0 flex-1">
      ...
      <Input
        ...
        className="h-11 rounded-2xl border-white/10 bg-white/[0.045] pl-10 pr-20 text-sm shadow-none backdrop-blur-xl"
        placeholder="Rechercher une fonction, un client, un document..."
      />
      <span className="pointer-events-none absolute right-4 top-1/2 hidden -translate-y-1/2 rounded-lg border border-white/10 bg-white/[0.055] px-2 py-0.5 text-[10px] font-medium text-[var(--muted)] sm:block">Ctrl K</span>

      {open ? (
        <div className="absolute left-0 right-0 top-14 z-[100] max-h-[70vh] overflow-y-auto rounded-[28px] border border-white/10 bg-[#0B0E14]/95 p-2 shadow-[0_32px_100px_rgba(0,0,0,0.55)] backdrop-blur-2xl">
          ...
          {groups.map((group) => {
            ...
            return (
              <div key={group.category} className="py-1">
                <div className="flex items-center gap-2 px-3 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-[#D6B56D]/70">
                  ...
                  {group.rows.map((result) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition", active ? "bg-white/[0.075]" : "hover:bg-white/[0.045]")}
                      >
                        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.055] text-cyan-200">
