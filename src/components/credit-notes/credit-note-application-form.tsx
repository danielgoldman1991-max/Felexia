...
export function CreditNoteApplicationForm({ creditNote, invoices }: { creditNote: CustomerCreditNoteRecord; invoices: InvoiceRow[] }) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="flex justify-end gap-3"><Link href={`/facturation/avoirs/${creditNote.id}`}><Button type="button" variant="secondary">Annuler</Button></Link><Button disabled={pending || total <= 0 || total > creditNote.available_amount}>Affecter l&apos;avoir</Button></div>
    </form>
  );
}
